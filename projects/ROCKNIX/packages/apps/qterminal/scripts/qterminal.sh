...
### Under gamescope the keyboard still works, but the workspace juggling below
### does not: there is no IPC and no tiling, and the compositor fullscreens
### qterminal by itself. So keep wvkbd, drop the sway half.
if [ "$(compositor)" != "sway" ]; then
  ...
  /usr/bin/wvkbd-mobintl -H 380 -L 380 -fg 6b6b75 -fg-sp 6b6b75 -bg 1d1d1d \
    ...
    -l full,nav,special &
  cleanup_osk_gamescope() {
    pkill wvkbd-mobintl 2>/dev/null
    set_setting "rocknix.touchscreen-keyboard.enabled" "${prior_tskb:-1}"
  }
  trap cleanup_osk_gamescope EXIT INT TERM
  cd ~/
  /usr/bin/qterminal
  exit $?
fi
